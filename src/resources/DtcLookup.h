/*
 * DtcLookup.h (DTC mapping)
 *
 * Derived from the `pcodes` mapping in `obd2_codes.py` from the pyOBD project:
 ...
 *
 * License (upstream): GNU General Public License, version 2 or (at your option) any later version.
 * This derived file is distributed under the GNU GPL v3 as part of this project, as permitted by
 * the upstream “or later” licensing option.
 *
...
inline std::string_view lookup(std::string_view code) {
    auto it = DTC_MAP.find(code);
    return (it != DTC_MAP.end()) ? it->second : "Unknown Trouble Code";
}
